		...
		if (weather.equals("SUN")) {
			try {
				Simulation.writeToFile(
						craftID() + "SUN-> the SUN and her eyes were the only fire, my dead wife loved the sun\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
			...
			try {
				Simulation.writeToFile(craftID()
						+ "FOG-> Am I at a WWE event? it's so fogy, is THE UNDERTAKER 'bout to make his entrance?!!\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
